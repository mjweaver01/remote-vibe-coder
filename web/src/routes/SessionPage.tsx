  ...
  return (
    <main className="page page-session">
      <Topbar
        ...
        trailing={<IconButton icon={Trash2} label="Kill session" tone="danger" size="sm" onClick={handleKill} />}
      />

      <nav className="tabstrip" aria-label="Session views">
        <NavLink end to={`/s/${sessionId}`} className={({ isActive }) => `tab-btn${isActive ? ' is-active' : ''}`}>
          <SquareTerminal size={14} aria-hidden="true" />
          <span>Terminal</span>
        </NavLink>
        <NavLink to={`/s/${sessionId}/files`} className={({ isActive }) => `tab-btn${isActive ? ' is-active' : ''}`}>
