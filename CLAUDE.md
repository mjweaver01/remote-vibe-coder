...
- **Service worker (`web/public/sw.js`) requires explicit cache-busting on edits.** Browsers compare SW bytes on each reload to decide whether to install a new one. We force this by (a) serving `/sw.js` with `Cache-Control: no-cache, no-store, must-revalidate` from a dedicated route in `src/server.ts`, and (b) keeping a `const SW_VERSION = "<date>.<letter>"` at the top of `sw.js`. **Whenever you change anything in `sw.js`, bump `SW_VERSION`** (e.g. `2026-05-12.b` → `2026-05-12.c`, or roll the date) so the byte change guarantees a fresh install. Without the bump, an idle byte-identical edit can fail to roll out even with no-cache headers if a caching proxy/CDN sits in front.
- **Notification scoping is enforced in the SW, not the server.** The server fans pushes to all subscribers; `sw.js` filters by checking `self.clients.matchAll()` against `payload.sessionId` — only shows banners for sessions with an open tab on this device, and only when that tab isn't currently visible/focused. If you add a new push payload type, decide explicitly whether it should be scoped to a `sessionId` or fall through unconditionally.
