...
export async function startServer(opts: ServerOptions): Promise<RunningServer> {
  ...
  app.get("/manifest.webmanifest", async (c) => {
    ...
    const manifest = {
      ...
      theme_color: "#0a0a0a",
      // PNG only — iOS Safari falls back to a page screenshot if it picks an
      // SVG icon (it advertises `sizes: any` but can't reliably rasterize it
      // for the home-screen tile). Keep the SVG link in index.html for the
      // browser tab favicon, but don't list it in the manifest.
      icons: [
